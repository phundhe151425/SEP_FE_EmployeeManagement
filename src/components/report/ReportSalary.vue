        ...
        <tr v-for="(user, indexLog) in users" :key="indexLog">
          ...
          <td
              ...
              :class="{
                ...
                disable: (year != currentDate.getFullYear()) || (year == currentDate.getFullYear() && currentMonth <= (currentDate.getMonth() + 1)-2 )
                || (year == currentDate.getFullYear() && currentMonth <= (currentDate.getMonth() + 1)-1 && currentDate.getDate() > closingDate ),
              }"
...
export default {
  ...
  methods: {
    ...
    tongSoNgayTrongThang() {
      // Lưu ý: Tháng trong JavaScript bắt đầu từ 0, nên cần trừ đi 1
      // var ngayDauThang = new Date(this.currentYear, this.currentMonth - 1, 1);
      var ngayCuoiThang = new Date(this.currentYear, this.currentMonth, 0);

      // Sử dụng phương thức getDate() để lấy ngày cuối cùng của tháng
      this.totalDayInMonth = ngayCuoiThang.getDate();
      console.log(this.totalDayInMonth)
